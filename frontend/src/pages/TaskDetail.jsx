...
import {
  ...
  XCircle, Clock, AlertCircle, MessageSquare
} from 'lucide-react'
...
const TABS = ['Genel', 'Brief', 'Aşamalar', 'Üyeler']

// ─── Genel Tab ────────────────────────────────────────────────────────────────
function GeneralTab({ task, members, onUpdate }) {
  ...
  const save = async () => {
    setSaving(true)
    try {
      await tasksApi.update(task.id, { ...form, current_owner_id: form.current_owner_id || null })
      onUpdate()
      setEditing(false)
  ...
  const priorityOptions = [
    ...
    { v: 'high', l: 'Yüksek' }, { v: 'urgent', l: 'Acil' }
  ]
  const statusOptions = [
    { v: 'todo', l: 'Bekliyor' }, { v: 'in_progress', l: 'Devam Ediyor' },
    { v: 'completed', l: 'Tamamlandı' }, { v: 'blocked', l: 'Engellendi' }
  ]
...
function BriefTab({ taskId, members }) {
  ...
  const [mode, setMode] = useState('view') // view | edit | submit | respond
  const [form, setForm] = useState({})
  ...
  const startEdit = () => {
    setForm({
      ...
      budget: brief?.budget || '',
      timeline_notes: brief?.timeline_notes || '',
      notes: brief?.notes || '',
    })
    setMode('edit')
  }
  ...
  const saveBrief = async () => {
    ...
    try {
      if (brief) {
        const { data } = await briefsApi.update(brief.id, form)
        setBrief(data)
      } else {
        const { data } = await briefsApi.create(taskId, { ...form, created_by: currentUser.id })
        setBrief(data)
      }
      setMode('view')
    } finally { setSaving(false) }
    ...
    return (
      <div className="space-y-4">
        ...
        {[
          ...
          { field: 'timeline_notes', label: 'Zaman Çizelgesi', type: 'textarea', placeholder: 'Önemli tarihler, milestonelar...' },
  ...
  if (mode === 'submit') {
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-gray-500">Brief'i onaylamasını istediğiniz kişileri seçin. Tüm onaylayıcılar onaylarsa brief onaylanmış sayılır.</p>
          ...
          {members.filter(m => m.id !== currentUser?.id).map(m => (
            <label key={m.id} className="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50">
              <input
                ...
                onChange={() => setSelectedApprovers(prev =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-500">
            {brief.creator_name && `${brief.creator_name} tarafından oluşturuldu`}
          </span>
        </div>
        <div className="flex gap-2">
          ...
          {brief.status === 'draft' && (
            <button onClick={() => setMode('submit')} className="btn-primary flex items-center gap-2">
              <Send size={14} /> Onaya Gönder
            </button>
          )}
          {brief.status === 'rejected' && (
            <button onClick={handleRevise} disabled={saving} className="btn-primary flex items-center gap-2">
              <RotateCcw size={14} /> Revize Et
            </button>
          )}
        </div>
      </div>
      ...
      {/* Onay durumu */}
      {brief.approvals && brief.approvals.length > 0 && (
        <div>
                ...
                <div key={a.id} className="flex items-start gap-3 p-3 border border-gray-100 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-medium ${
                        ...
                        a.status === 'rejected' ? 'text-red-600' : 'text-amber-600'
                      }`}>
                  ...
                  {a.responded_at && (
                    <span className="text-xs text-gray-400 flex-shrink-0">
                      {new Date(a.responded_at).toLocaleDateString('tr-TR')}
                    </span>
                  )}
                </div>
              )
            ...
            <div className="mt-4 p-4 border-2 border-indigo-100 rounded-xl bg-indigo-50">
              ...
              <div className="space-y-3">
                <div className="flex gap-3">
                  <label className={`flex-1 flex items-center justify-center gap-2 p-2.5 rounded-lg border-2 cursor-pointer transition-all ${respondData.decision === 'approved' ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 bg-white text-gray-600 hover:border-green-300'}`}>
                    <input type="radio" name="decision" value="approved" checked={respondData.decision === 'approved'} onChange={() => setRespondData(d => ({ ...d, decision: 'approved' }))} className="hidden" />
                    <CheckCircle2 size={16} /> Onayla
                  </label>
                  <label className={`flex-1 flex items-center justify-center gap-2 p-2.5 rounded-lg border-2 cursor-pointer transition-all ${respondData.decision === 'rejected' ? 'border-red-500 bg-red-50 text-red-700' : 'border-gray-200 bg-white text-gray-600 hover:border-red-300'}`}>
                    <input type="radio" name="decision" value="rejected" checked={respondData.decision === 'rejected'} onChange={() => setRespondData(d => ({ ...d, decision: 'rejected' }))} className="hidden" />
                ...
                <textarea
                  ...
                  value={respondData.comment}
                  onChange={e => setRespondData(d => ({ ...d, comment: e.target.value }))}
                />
                <button onClick={handleRespond} disabled={saving} className="btn-primary w-full">
                  {saving ? 'Kaydediliyor...' : 'Kararı Gönder'}
                </button>
              </div>
            </div>
          )}
...
function StagesTab({ task, members, onUpdate }) {
  ...
  const complete = async (stageId) => {
    await stagesApi.complete(stageId)
    onUpdate()
  }

  const start = async (stageId) => {
    await stagesApi.start(stageId)
    onUpdate()
  }
  ...
  const statusConfig = {
    pending: { label: 'Bekliyor', cls: 'bg-gray-100 text-gray-500', dot: 'bg-gray-300' },
    active:  { label: 'Aktif', cls: 'bg-indigo-100 text-indigo-700', dot: 'bg-indigo-500' },
    completed: { label: 'Tamamlandı', cls: 'bg-green-100 text-green-700', dot: 'bg-green-500' },
  }
            ...
            return (
              <div key={s.id} className="flex items-center gap-1 flex-shrink-0">
                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border ${
                  ...
                  {s.assignee_name && (
                    <MemberAvatar name={s.assignee_name} color={s.assignee_color} size="xs" />
                  )}
                </div>
                {i < stages.length - 1 && (
            ...
            <div key={s.id} className={`rounded-xl border-2 p-4 transition-all ${
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {s.assignee_name && (
                    <MemberAvatar name={s.assignee_name} color={s.assignee_color} size="xs" showName className="mt-1" />
                  )}
                  {s.completed_at && (
                    <p className="text-xs text-green-600 mt-1">
                      {new Date(s.completed_at).toLocaleDateString('tr-TR')} tarihinde tamamlandı
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {s.status !== 'completed' && (
                    <button onClick={() => { setEditingId(s.id); setEditForm({ title: s.title, description: s.description || '', assignee_id: s.assignee_id || '' }) }} className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors">
...
function MembersTab({ task, members, onUpdate }) {
  ...
  const addMember = async (memberId) => {
    await tasksApi.addMember(task.id, memberId)
    onUpdate()
  }

  const removeMember = async (memberId) => {
    if (!confirm('Bu üye görevden çıkarılsın mı?')) return
    await tasksApi.removeMember(task.id, memberId)
    onUpdate()
  }
...
export default function TaskDetail() {
  ...
  const loadTask = useCallback(() => {
    Promise.all([tasksApi.getById(id), membersApi.getAll()])
      .then(([tRes, mRes]) => { setTask(tRes.data); setMembers(mRes.data) })
      .finally(() => setLoading(false))
  }, [id])
    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="flex border-b border-gray-200 mb-6 gap-1">
        {TABS.map((tab, i) => (
          ...
          >
            {tab}
            {tab === 'Aşamalar' && task.stages?.length > 0 && (
              <span className="ml-1.5 text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full">{task.stages.length}</span>
            )}
            {tab === 'Üyeler' && task.members?.length > 0 && (
              <span className="ml-1.5 text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full">{task.members.length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="card p-6">
        {activeTab === 0 && <GeneralTab task={task} members={members} onUpdate={loadTask} />}
        {activeTab === 1 && <BriefTab taskId={id} members={members} />}
        {activeTab === 2 && <StagesTab task={task} members={members} onUpdate={loadTask} />}
        {activeTab === 3 && <MembersTab task={task} members={members} onUpdate={loadTask} />}
      </div>
    </div>
  )
}
